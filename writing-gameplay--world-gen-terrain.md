...
- **Laying a thin skin prop over a graybox ground surface (the slab keeps its collision, a plate prop carries the picture) has one silent failure mode: a skin thicker than the markings standing on it SWALLOWS them whole.** A 30 mm road skin over a slab erased every 14 mm crossing on the map; both props were correct on their own, still placed, still counted, still in budget, and invisible, because every check measures one prop in isolation. A skin's thickness is a budget the things standing on it spend: before picking one, list every prop that shares its datum, take the thinnest, publish the chosen thickness as ONE constant every placer reads, and raise anything standing on the skin by exactly that. Second half: a material applied to HALF of a continuous surface advertises the boundary, so the hard seam between skinned and unskinned halves is more visible than the flat surface was; if the seam falls anywhere a camera goes, skin the whole surface or none of it.
