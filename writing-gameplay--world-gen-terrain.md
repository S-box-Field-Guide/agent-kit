...
- **An editor generate tool leaves a transient terrain-root GameObject in the open scene — it
 ...
 loop, `set_game_object {enabled:false}` on the terrain root (don't delete — it's session-
 ...
 NOT a bare list — and its `name` filter is a substring, so a shared-prefix query also
 matches the terrain root; filter by exact prefix before deleting so you don't nuke it.
...
- **A low-frequency "mountain-mass" mask at wavelength >> world width makes the WHOLE
 ...
 mountainous by pure seed luck, with no apron↔massif split.** An early HeightPass used
 massScale = 1.5× world width; across the 192 m world that
...
- **A translucent water film over BRIGHT WARM terrain desaturates to muddy grey, not blue —
 ...
 it further.** A first water.vmat (tint blue, TextureTranslucency 0.55 = 55% opacity,
...
- **Snow (and any climate band) must gate on ABSOLUTE altitude in metres, never elevation
 FRACTION (h / amplitude) — the fraction gate silently snows low-amplitude worlds.** An early pass
 gated snow at StrataSnowFrac 0.82 of amplitude; on desert_mesa (amp 18) that is 14.8 m, so its
 16–18 m mesas got snow-capped — an arid desert with snow. The fix makes snow a BIOME keyed on
...
- **A monotonic invariant proven on a FLOAT construction can still be violated on the QUANTIZED
 grid the audit reads — enforce it on the final grid, not just at authoring.** Rivers clamp
...
- **Nearest-centerline corridor stamping leaves a step DISCONTINUITY where two flattened
 ...
 Roads flatten each cell to their nearest A*-centerline's smoothed height; where two corridors
...
- **Steepest-descent rivers stall into mid-slope tarns on a rugged/ridged crest; the fix is more
 ...
 more octaves or a deeper carve.** A first river_valley run reached the sea on only 2 of 4
...
- **A per-cell WHITE-noise hash is the WRONG driver for pixel-art terrain shade choice — it reads
 ...
 The defect: `hash = LatticeValue(x,y,seed); mat = hash<0.5 ? GrassLight : GrassDark`
 gave a 50/50 checkerboard across the whole apron. The fix is
 ...
 earlier "dither+banding pairing" note: the ABSOLUTE-height alt-shade band that makes cliff
...
- **Regenerating the strata atlas PNG at a NEW cell-grid size (8x6 -> 8x8) is safe through the
 ...
 A later pass appended 6 feather-band mid-stop cells, growing the atlas from 26->needs-4-rows. The append
 worked with zero runtime breakage because the atlas helper's `CellCenterUv` divides by `Cols`/`Rows` (const,
 ...
 needed (and `asset_compile` on the atlas png actually errors "no source file / atlas.jpg"
...
- **A runtime world root torn down with `GameObject.Destroy()` KEEPS RENDERING in EDIT mode — the
 ...
 `previousRoot?.Destroy()` then built a new world root; in edit mode `scene_tree` then showed TWO
 world roots, each with 923 children, both Enabled. Identical specs coincide exactly (invisible —
 determinism/snapshots look fine); a CHANGED slider (amplitude/scale/sea) leaves the previous world
 showing THROUGH the new one — this is the visual inconsistency you see when sweeping sliders and
 regenerating in the editor. Proof: generate amp80 then amp10 + screenshot = the tall amp-80 mountain
 renders through the flat amp-10 plain. Same family as the grid-vs-collider "static repointed
 mid-play" bug. FIX: `foreach(root named <worldRoot>) root.DestroyImmediate()` (synchronous, valid in both
 ...
 grid-hash McpTool that also returns `worldRootCount` + `chunkGOs` vs `expectedChunks`, asserted
...
- **A deterministic content hash for "same spec = same world" byte-identity checks must use a
 ...
 would make two editor sessions disagree. the project's grid-hash tool folds the persistent WorldGrid
...
- **Flat-topped MESAS from a heightfield need TWO terraces, not one: terrace a SMOOTH low-frequency
 ...
 mesa lane first tried terracing the detailed massif — gave concentric "wedding-cake" contour rings
...
- **Two water audits can CONFLICT at the sea boundary, and the resolution must be keyed on the QUANTIZED
 step, not the float height.** the project later added `water_contained` (no water surface above an
...
- **Altitude-driven biome gradients: derive EVERY climate band from ONE effective-temperature lapse
 ...
 slider space.** The rule being enforced was "desert must never abut snow": tEff = Temperature - h/SnowSpanM
...
- **A per-CELL minimum-depth gate on flood-filled water SHREDS lake sheets — depth-gate per BASIN,
 ...
 films at unique heights ringing the basin (507 offenders on the live default
...
- **Two special-case water rules that are each correct alone can collide INSIDE a feature: "below-sea
 ...
 dips under the sea.** The earlier override (quantized floor below the sea step ⇒ surface = sea — needed
...
- **VISUAL adjacency laws ("X must never read as touching Y") need a PLAN-distance mechanism, not just
 ...
 off the dilated "environment height".** An earlier pass eased sand→grass with altitude; sheer
