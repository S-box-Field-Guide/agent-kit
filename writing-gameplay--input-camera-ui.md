...
- **A console or editor text field holding keyboard focus starves EVERY `Input.*` read in game code — a debug camera in that state is pixel-identical to a broken one.** Typing a command into the console is exactly what moves keyboard focus off the game, so every subsequent WASD lands in the text box; that's why such reports are INTERMITTENT — the same feature works from its keybind and fails from the console, nothing differing but where the caret is. A headless/MCP play session drives the build through the console, so it's PERMANENTLY starved and can never prove a device-input path. Game code CANNOT take focus back (`InputRouter.KeyboardFocusPanel` is `internal`); it can only detect it: `Application.IsFocused` (the window — false is conclusive, true isn't, a panel can hold the caret), `Input.Suppressed`, `Input.MouseCursorVisible`/`Mouse.Active`/`Mouse.Visibility`. Fix the SILENCE, not the focus: have a live feature notice it's read nothing for a few seconds and log once with the focus reads attached; put "seconds since last input" + "ever been fed" on its state command.

- **`Input.Keyboard.Pressed` fires AGAIN on OS key auto-repeat, not only the physical down edge — so a toggle written on `Pressed` flips TWICE for any key held past the ~500ms Windows repeat delay** (`if (Pressed("J")) open=!open;` opens, then closes itself half a second later; users who think the key is broken hold it longer, worsening it). Safe toggle = true down edge with previous-frame state: `bool d=Input.Keyboard.Down(k); if(d && !_wasDown) toggle(); _wasDown=d;` — `Down` stays true across the whole hold including between repeats, so a repeat can never re-toggle at any hold length (a time-based debounce fails on long holds). `Pressed` is fine where re-fire is wanted (menu stepping, list paging); audit one-shot raw-letter actions that spend money or mutate state. -> [fix article](/fix/input-pressed-refires-on-key-repeat)

- **Every `ICameraModifier` in the scene runs against whichever camera is currently DRAWING, in ascending `CameraOrder`: a second camera (overhead, spectate, killcam) inherits every modifier written for the primary camera, even though the inspector shows the second camera's OWN transform as correct.** An orthographic planning camera placed dead centre above a building, position and rotation both confirmed by query, rendered pure sky: a player-controller modifier and a first-person viewmodel's camera modifier both ran against the main camera, and the instant the second camera became the drawing camera it inherited the player's look angles instead of its own. Neither camera reports wrong, which is what makes this nasty. Diagnostic: flip the suspect camera to perspective, a horizon line (or a viewmodel hanging mid-frame) proves something else is steering the view. Cure: claim the intended view with your OWN `ICameraModifier` at a HIGHER `CameraOrder` than anything else, highest order runs last and wins, rather than disabling modifiers that may own state you still need.

- **An `ICameraModifier` writes to every camera that DRAWS, including the editor viewport camera during play-in-editor (a real `CameraComponent` on a GameObject named `editor_camera`), not just the camera its component belongs to.** A modifier that writes only SOME fields makes the viewport obey some instructions and ignore others: a first-person `ModifyCamera` that set `view.Rotation` but not `view.Position` had a camera tool's position honoured and its rotation discarded, so yaws 0, 90, 180, and 270 from one spot rendered the same frame. A transform readback cannot see this: the object reports the `WorldRotation` you just set, because the override happens later during view composition, downstream of the object. The only witness is a rendered frame: shoot the same position at two bearings 180 degrees apart and compare with a small mean-luminance signature (a 32x18 downsample plus a difference floor), not a byte compare (temporal AA and exposure drift make identical views differ). Fix: open every `ModifyCamera`/`PostCameraSetup` body with `if ( !camera.IsValid() || !camera.IsMainCamera ) return;`. Fingerprint: rotation that works in edit mode and freezes once play starts is a project component, not an engine defect. Sibling of the second-camera case above where the victim is an in-game overhead or spectate camera.
