...
- **Mixamo mocap retargets onto the NPC lane by REST-DELTA + PARENT-RELATIVE
  ...
  dq_bone`, NOT the raw world delta. The rig library's Poser composes bone_world = dq_parent @
  ...
  P.arm/P.REST_Q/P._last_q — no rig-library edit, keeps hemisphere continuity).
...
- **Physics shape coords are in the SAME UNIT as the mesh+bones BEFORE the
  ...
  inches.
- **Opt-in additive authoring without editing the read-only rig library:**
  the NPC rig library's `write_vmdl_physics` calls the rig library's `write_vmdl` to author the base
