...
> advice; unverified material is either held back or carries an explicit **Unverified:** label. The sync appends new bullets here.
...
## Multi-agent & config discipline

- **Config placeholders go in docs, not identity fields.** A project `Org` field parked
  as an uppercase `TODO_PLACEHOLDER` breaks engine bootstrap — Org must be a valid
  lowercase package ident. Keep a real lowercase value (e.g. `local`) for dev and record
  the TODO in a doc instead.
- **Agents never stash/reset/checkout broad paths in a shared worktree.** A bare
  `git stash` run to isolation-verify one agent's build captures other agents' in-flight
  WIP — there is no per-agent stash scoping. Scope it (`git stash push -- <your files>`)
  or verify isolation via a detached-worktree overlay build (copy your owned files over
  HEAD in a temp worktree and build there). Explicit git paths only — never `git add -A`
  in a shared tree; milestone sweeps otherwise capture other agents' half-written files.
