...
- **Autopilot progress metrics must be state-gated** - sampling "max height gained" across a
 whole routine counts the ballistic kick-off apex as climb progress (test-lab run 1 read
 7.2 m on a 4.1 m tower). Gate progress samples on the state that earns them (Climb/WallRun
 only).

- **The editor console buffer (2000 entries) rolls over during long pilot sweeps** - per-attempt **CORRECTION (engine 26.07.22):** a direct measurement put the ring at ~502 entries, not 2000 — treat the depth as build-dependent and never assume a specific size; harvest the console tail every few seconds and accumulate de-duplicated lines so a roll between polls can't lose data.
...
- **`SkinnedModelRenderer.SceneObject` is null until the renderer first goes live, so a spawn-time `Flags.CastShadows` (or similar render-flag) write silently no-ops** — nothing errors, and an unpredictable subset of spawned objects stays stuck at the SceneObject's default. Don't trust the write: read back the state that ACTUALLY landed (not the value you asked for) and retry on a staggered sweep across following frames until the read-back matches. A/B verified at 120 spawned NPCs — retry-until-confirmed converges every one; fire-and-forget leaves a random subset wrong.
...
- **`GameObject.Destroy()` is DEFERRED to the end of the frame — an object you just destroyed still exists, still answers, and still HOLDS any exclusive claim it owns (`IsMainCamera`, a singleton/cursor claim, a registry slot) for the rest of that frame.** A debug camera destroyed its own camera object then set the player camera's `IsMainCamera` true; for the rest of that frame BOTH cameras reported `IsMainCamera==true` — the exact ambiguous state the swap avoided. Rule: for any contract-exclusive claim, clear it on the OUTGOING owner BEFORE calling Destroy, in the same breath the incoming owner takes it — "destroy then fix up whoever's left" is safe only for claims the destroy itself releases, and a deferred destroy releases nothing until frame end. The same deferral makes a same-frame leak census LIE (a by-name object count printed `rigs=1` with the rig OFF, right after teardown); distinguish a real leak from a pending destroy by re-reading on a LATER frame — a real leak survives it, a pending destroy doesn't.

- **A component injecting movement input for automated testing must sit FIRST in the scene hierarchy AND write on both `OnUpdate` and `OnFixedUpdate`, or every movement phase reads zero while looking still works.** `Input.AnalogMove` is computed once per frame from the movement actions before components run, so setting the underlying Forward/Back/Left/Right actions afterward moves nothing that frame: write `Input.AnalogMove` directly for held movement (`Jump` and `Duck` stay real action reads via `Input.SetAction`). Components run in GameObject order, so a harness on or under the controlled character runs AFTER the stock mover and every write lands one frame late, then gets wiped when the next frame rebuilds input: put the harness on the FIRST object in the scene. Movement is consumed on the fixed step while looking is consumed on the per-frame update, and all of a frame's fixed steps are done by the time `OnUpdate` runs, so a harness that is first but injects only from `OnUpdate` passes look and camera phases and reads exactly zero on every movement phase. Fix: inject from both `OnUpdate` and `OnFixedUpdate`.

- **Update order is a property of the AUTHORED scene: a component added at runtime (`Components.GetOrCreate<T>()`) goes on the END of the update list no matter which GameObject it sits on, so per-frame INPUT written from it lands too late for the movement code that reads it the same frame.** The failure is completely silent: no error, no null, no log. A runtime-created component on the first object wrote `Input.AnalogMove` from `OnUpdate` and `OnFixedUpdate` exactly like the authored component beside it and the player did not move, while `EyeAngles` (a direct property write that depends on no ordering) landed the whole time: the component runs, and it runs too late. Fix: anything that injects per-frame input must be REACHED from a component the scene authored. Creating it at runtime is fine; being updated by the engine at runtime is not, so have the authored component's `OnUpdate`/`OnFixedUpdate` call the created one's tick methods, with a suspend flag so the two never write movement in the same frame. Authoring it into the `.scene` file also works and costs an editor restart, because a live editor does not reload a scene whose file changed underneath it.

- **A scene-authored component cannot resolve, in its own `OnStart`, a component that ANOTHER component creates inside ITS `OnStart`: authored-BESIDE is not authored-BEFORE.** The binder starts into a world where its target has been asked for and not yet built, binds to null, and stays silent behind one warning line at the top of the log, so a whole feature does nothing while everything it depends on works. Cure: bind LAZILY whenever the target is CREATED rather than authored. Retry every quarter second until it resolves, and warn once if it never does. Same underlying ordering fact as the runtime-created-component trap: `OnStart`-created components do not exist yet for anything else's `OnStart`.
