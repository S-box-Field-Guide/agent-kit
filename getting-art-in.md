...
- **Animated sub-parts need their origin at the joint.** Export limbs/rotors as separate
 ...
 parent them to the body GameObject and rotate `LocalRotation`. Used for the character's
...
- **Scene triangle overload: census (tris × instances) BEFORE building decimation
 ...
 batching.
- **Blender OBJ import `up_axis='Y'` puts Y→Z rotation in `matrix_world`, not vertices** — raw `.co.z` reads the file's horizontal axis. Apply transforms after import (`bpy.ops.object.transform_apply`) to bake world-Z-up into vertex coords. **Also (same mechanism):** assigning to the imported object's `rotation_euler` directly WIPES the axis-conversion rotation the OBJ importer stored in `matrix_world`, because the assignment REPLACES the whole rotation rather than composing with it — a placement script that sets `obj.rotation_euler = (0, yaw, 0)` on a freshly imported module silently discards the Y-up→Z-up conversion and every placed module renders tipped over, with every numeric gate (colliders, bounds) still passing. Compose onto the existing matrix, or apply the import transform first, instead of overwriting `rotation_euler`.
...
- **A Cycles spot light placed exactly at an emissive housing's surface sits INSIDE the housing geometry and loses roughly half its light to self-shadowing.** Placing a render-only light at the lamp bank's own mesh position (the natural first guess: "put the light where the lamp is") puts its origin coincident with or inside the housing's own solid volume, and the housing occludes half the light's sphere before it ever reaches the scene. **Render-rig convention, now standard for this shop's arena renders: stand the spot off along its own aim vector by 1 m** (`loc = lamp_bank_pos + aim.normalized() * LAMP_STANDOFF`, `LAMP_STANDOFF = 1.0`) — a render-only nudge, safe because the shipped in-engine lights are shadowless (`spot.Shadows = false`), so there is no gameplay-visible light position that has to stay in sync with the render rig's. Sibling convention to the absolute-camera-coordinates render-rig entry above: render rigs get their own small, documented fudge constants that never touch the shipped asset's real data.

- **`bpy.types.Mesh.validate()` silently DELETES degenerate faces, which can hide a real sliver bug in a sealed-revolve collision mesh while the reported tri count still looks right.** A generator that builds a collision mesh by revolving a profile around an axis can produce a genuinely degenerate ring when the profile's radius collapses toward zero (a sealed cap, a pinched neck); calling `validate()` (directly, or implicitly via an export/cleanup step that calls it) quietly drops those faces instead of erroring, so a script that only checks the FINAL tri count against an expected range sees a plausible number and moves on — the sliver is real, it just isn't visible in the surviving mesh's stats. Guard on the actual geometric condition instead of the downstream symptom: check for BOTH radii near zero at a revolve step (the pinch condition itself), not on band length or face/tri count, which `validate()`'s cleanup can make look normal even when a sliver was silently removed.

- **A wheel-arch collider-clearance law (forbidding collider volume inside a cylinder around each hub) binds much harder on a kit with SHORT overhangs than on the reference kit it was proven against, and copying the reference kit's part plan onto a shorter body is not a "nudge the box" fix — it can eliminate an entire collider's worth of legal room.** On the reference (long-nose) kit the arch cylinder ends well short of the body's own nose, so the law is invisible while authoring it; on a kit with a much shorter front overhang the cylinder reaches almost to the body edge, and consequences cascade: a nose-block collider is forced down to a sliver, corner colliders have to sit inside the bumper cover's own depth instead of wrapping forward with the visible geometry, and a rear quarter that used to get two colliders may legally fit only one because there is no room behind the wheel at all. **Rule: derive each arch cylinder from THIS kit's own wheelbase and wheel radius before placing a single collider, and accept a smaller collider set rather than shaving the law** — a bigger wheel radius also raises the arch cylinder's top, so the two dimensions have to be checked together, not assumed from the last kit that used the same rig.

- **Any authored GEOMETRY (a rub strip, a paint band, a lower-body trim block) that runs along a vehicle's flank and crosses a wheel opening is the same defect as a COLLIDER crossing it, and a coplanar/arches sanity pass should flag both the same way.** A strip that continues straight through the arch at hub height, a trim block whose outboard face sits flush with the tyre's own face (i.e. directly over the wheel), and a proud two-tone paint band crossing the arch were all caught by the same census check: any face standing outboard of the tyre's outer cap inside the wheel silhouette. Fix pattern: any strip/band/block running along the flank either stops at each opening or gets boolean-cut to follow the arch, the same way the arch's own cutout is authored. If the cutting helper takes a hub position, pass it in the SAME coordinate space the helper expects (part-local, not the panel's own local origin) — passing the wrong origin cuts a hole in the wrong place and the resulting ring can end up covering the wheel it was meant to frame.

- **A primitive authored with a rotation parameter (`rot=`) spins about its OWN object origin — wherever it was placed — not about the part's intended pivot.** A raked hood authored as a box at some location with a rotation applied see-saws about its own middle: the rear edge rises above the cowl by exactly as much as the nose falls, producing half the intended wedge in each direction instead of a clean rake. To rotate correctly about a chosen pivot, apply the rotation to the PLACEMENT coordinate before building the box there (rotate the position vector by the desired angle around the pivot, then place the unrotated-relative-to-itself box at that rotated position with the same rotation baked into its own orientation) rather than placing the box first and rotating it in place. A quick sign check that generalizes: a slab's local "up-ish" axis should map to `(sin(angle), 0, cos(angle))` for a rotation about Y — verify a known-good rotated part in the same generator already satisfies that relation before trusting a new one. Related and easy to forget: **a rotated box's own axis-aligned bounding box is trigonometric**, `cos(|angle|)*length/2 + sin(|angle|)*thickness/2`, not the box's declared half-length — a rear-bumper corner return rotated 28 degrees measured noticeably wider than its declared half-width and quietly became the widest point on the whole vehicle.

- **Copying a front-end part's generator code to build the matching rear part does not mirror correctly just because the coordinates are symmetric — a "proud" or "outward" sign convention that was correct at the front is frequently backwards at the rear, and no numeric gate can see it.** A tail-lamp cluster built by copying the front lamp's code came out buried INSIDE the tail panel, because "proud" at the tail is the opposite sign from "proud" at the nose. Every automated check passed: colliders were correct, budget was correct, every band and gate cleared — the kit shipped with no visible rear lights and nothing in the numeric pipeline caught it, because "the part exists at a valid position with a valid collider" is exactly what a buried part also satisfies. **Rule: render the rear (and any other mirrored face) and eyeball it before trusting a symmetric-code-reuse part is correct.** A fully assembled exterior render proves nothing about whether an internal or mirrored part is actually where the manifest claims it is — a dedicated reveal/cutaway render pass (hood off, panel off, wheel off, fully stripped) is the only offline evidence for that class of bug. **Related failure, same "no numeric gate can see it" shape but a WINDING bug instead of a sign-convention bug: a directional emissive face wound to face the wrong local axis passes every gate and glows away from its target instead of toward it.** A lamp lens quad's winding determines which way its face normal points, and therefore which way the self-illum shader reads as "lit" toward a viewer; a lens wound on the axis opposite the one its docstring and every consumer assume it faces still compiles, still exports, and still passes the coplanar/winding census — a flipped face is still a closed, correctly and CONSISTENTLY wound 2-manifold from a mesh-validity checker's point of view, because that class of check proves winding CONSISTENCY, not winding DIRECTION. It still renders an emissive surface, it just glows into the housing instead of out of it, on every placed instance across every scene that uses the module, and nothing about "the mesh is valid" catches a face that is validly wound the wrong way. **Rule: any directional surface feature (a lens, a sign face, a headlight cone) needs an explicit FACING assertion in the generator's gate, not just a validity assertion** — take the face's final normal and dot it against the documented aim axis, and fail the gate if the sign is wrong. This is the exact shape a sibling part in the same codebase already gates a different feature with (a radial facing check on a curved sponsor-board face); the fix is to apply the same one-line dot-product assertion to every directional emissive/facing feature a generator places, not just the one that already had it.
...
- **The engine ships render quality profiles (`core/cfg/quality_profiles.json`: `TextureQuality`/`PostProcessQuality`/`VolumetricFogQuality`/`ShadowQuality`, each Low/Med/High) that CAP what your scene asked for.** `DirectionalLight.ShadowCascadeCount` is a request, not a setting — its XML doc says "User settings will set a maximum" and `ShadowQuality` Low pins `r.shadows.csm.maxcascades 2`, so a scene authoring 4 cascades renders 2 for that player (same for `r.shadows.maxresolution`/`csm.maxresolution`/`quality`). Measured cascade cost is the cost at the MEASURING machine's engine setting, not a portable constant. These are client-GLOBAL console state — a preset that writes them and doesn't restore leaves the next perf measurement grading a config nobody pinned.
- **Post-processing is opt-in PER CAMERA: AO/DOF/bloom/SSR/motion-blur convars tune passes that don't exist until the GAME adds the component.** `r_ao_quality` etc. are real and settable and do NOTHING alone — GTAO only runs when a `Sandbox.AmbientOcclusion` component sits on the camera. A camera created in code with `AddComponent<CameraComponent>()` and never given `Tonemapping`/`Bloom`/`AmbientOcclusion` renders flat, ungraded, un-occluded no matter how well lit. Full set: `Sandbox.{AmbientOcclusion,Bloom,Blur,ChromaticAberration,ColorGrading,DepthOfField,FilmGrain,MotionBlur,Pixelate,ScreenSpaceReflections,Sharpen,Tonemapping,Vignette}`. SSR follows the same rule: a non-zero `r_ssr_downsample_ratio` (stock default 2) does NOT mean SSR runs — it needs a `Sandbox.ScreenSpaceReflections` component; measured identical within noise across ratio 0/2/1 with no component attached. Rule: a non-zero engine convar is necessary, not sufficient — the component decides whether the pass runs.
- **A golden-hour sun at single-digit elevation puts NO direct light on a street canyon's ground, so nothing on that ground casts a visible shadow — and the cascaded shadow map is billed in full for producing a constant.** Shadow length on flat ground is `H / tan(elevation)`; at 7° a 14 m facade throws a 113 m shadow, and only the facade ACROSS the street blocks it (`length × sin(azimuth offset from street axis)`). This looks exactly like a bug and is not one: every shadow flag can be correct, the cascade well tuned, `CastShadows=true` everywhere, and the frame still has zero visible shadows (one crowd scene measured the cascade at 65–68% of every frame's draws while its only product was a shadow line partway down the facades). Fix is the sun angle, and a small azimuth offset buys far more ground light than a big elevation change (offset enters as sine, elevation as tangent): keep the sun within ~15° of the street axis, or accept a fully-shaded floor. Do the arithmetic before touching a renderer flag.
- **On `Sandbox.Tonemapping`, the exposure fields are INERT while auto-exposure is off — the curve `Mode` is the only dial that does anything.** `ExposureCompensation` (0→+2 stops) and a locked `MinimumExposure == MaximumExposure` window (1.0→3.4) both held a fixed camera at 24.25 mean luminance at EVERY value; `AutoExposureEnabled = true` is the switch that makes the exposure path live at all (same frame jumped to 96.01). So "lock exposure with Min==Max, then grade with ExposureCompensation" silently grades nothing — the lock and the compensation are mutually exclusive, not complementary. On a locked-exposure camera there is no post-process brightness knob; brightness must come from ambient/sky tint/envmap (`DirectionalLight.LightColor` is a hue, not an intensity, on this build).
- **A per-size `Material.CreateCopy` cache with a cap doesn't fail loudly when it fills: every later surface silently renders at the vmat's AUTHORED `g_vTexCoordScale`, which on a surface it was never meant for is a tile one or two orders of magnitude off and photographs as a flat tint.** `g_vTexCoordScale` can only be varied per surface by copying the material (a SceneObject render attribute writes without complaint and never reaches the shader), so a procedural world wanting one tile size across boxes of many sizes needs one copy per distinct quantised repeat, and the cache gets a cap. Two traps: the miss counter LIES BY UNITS (count distinct KEYS asked vs served, not consumers — 980 "missed boxes" was 41 missed keys against a cap of 48), and most consumers may not be drawn (read `Enabled` at REPORT time, not emit time — a graybox slot spends a key then gets `renderer.Enabled=false`). A cap raise is strictly additive (same deterministic fill order, stops later), so nothing already served is lost.
- **`FrameStats.Current` draw counts cannot A/B two procedural builds inside one play session: material copies from the previous build survive the rebuild and shift batch grouping, so two legs at IDENTICAL settings disagree by tens of draws** (measured 55 apart; restart the session per leg and the same comparison lands inside 9). Same checklist: freeze everything that moves (ambient vehicles/boats/crowd — with them running the number moved 90 draws and `ObjectsRendered` moved with it, the tell that a different set of objects was in frame), and print `TrianglesRendered` + `ObjectsRendered` beside the draw count every leg (two legs are only comparable when both match — the cheap proof the frustum contents didn't move under the experiment).

- **Texture detail authored finer than the on-screen scale a surface is actually seen at is destroyed by the mip chain before a player ever sees it — a prop that reads flat/plastic/untextured almost always HAS a texture whose contrast the mip chain has already thrown away.** Judge a sheet by its contrast AFTER an N:1 box downsample (what a mip level physically is), not the full-resolution image: under an 8x box downsample one measured set kept a fabric weave at 21% of its 1:1 contrast, paving 38%, wood grain 67%, iron 80% — ranking by full-res contrast (what your eye does opening the PNG) mis-orders which sheets survive. Author detail at the scale it will be viewed.

- **A `PointLight` born with `Shadows = true` contributes NOTHING to the frame on current builds — no light, no shadow, silently.** A runtime-created point light on a plain (non-static) root is equally black; toggling `Shadows` off restores its pool the same frame, back on kills it. A `SpotLight` with `Shadows=true` in the identical position works — the point-light six-face cubemap shadow path is refused wholesale while `SpotLight`'s single-view projected path is fine. For a directional lamp (street lamp, sconce) cast with a downward `SpotLight` (inner ~55 deg, outer ~80 deg, a few m up). Also: writing a light flag on a light BORN with the opposite value does nothing until the GameObject is disabled/re-enabled — A/B a light flag with a recreation cycle and an actual rendered frame, never a property read. -> [fix article](/fix/pointlight-shadows-renders-black)

- **`import_rotation` in a vmdl is honoured by the RENDER node and silently ignored by `PhysicsHullFile` (and `PhysicsMeshFile`): correcting a model's yaw with it fixes the picture and leaves the collision where it was.** A kit imported 90 degrees off; adding `import_rotation` fixed every render mesh but did NOT touch a `PhysicsShapeList`/`PhysicsHullFile` node reading the same source file, because that node does not consume the property: no error, no compile warning, the model builds clean. The result is worse than doing nothing, because a render-right and physics-wrong model photographs perfectly while a player walks straight through it, and the only symptom is a collision probe failing somewhere that looks unrelated to import. Cure: never rely on `import_rotation` alone for an off-axis import when a physics node shares the source file; bake a corrected, engine-frame COPY of the mesh that BOTH the render and physics nodes point at. General lesson: when two consumers read one asset property, verify each node independently, the one you did not look at is the one that silently did not get the fix.
